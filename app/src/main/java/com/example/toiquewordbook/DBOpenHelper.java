...
//    }
    ...
    {
        Log.e(TAG, "DB Copy Start");

        try
        {
            Log.e(TAG, "DB Copy executing0");
            InputStream inputStream = mContext.getAssets().open(mDBFileName); //여기서 ioexception.
            Log.e(TAG, "DB Copy executing1");
            String out_filename = mFolderPath + mDBFileName;
            Log.e(TAG, "DB Copy executing2");
            OutputStream outputStream = new FileOutputStream(out_filename);
            Log.e(TAG, "DB Copy executing3");
            byte[] mBuffer = new byte[1024];
            int mLength;
            Log.e(TAG, "DB Copy executing4");
            while ((mLength = inputStream.read(mBuffer)) > 0)
            {
                outputStream.write(mBuffer, 0, mLength);
            }
            Log.e(TAG, "DB Copy executing5");
            outputStream.flush();
            outputStream.close();
            inputStream.close();
